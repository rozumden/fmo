function out = average_fmo_vid(vid, fmos, n)
...
for i = 1:numel(fmos)
	...
	if isfield(fmos{i}.res,'fg')
		u = fmos{i}.res.fg;
	else
		u = fmos{i}.res.f;
	end
	m = fmos{i}.res.m;
	m = double(imresize(m, size(u,1)/size(m,1)) > 0.5);
	...
	dist = bwdist(BW);
	BW2 = BW | (dist <= (fmos{i}.Radius+15));
	u(~repmat(BW2, [1 1 3])) = 0;
	m(~repmat(BW2, [1 1 3])) = 0;
