function [D, idx] = pdist2( X, Y, metric, prop, val)
...
if nargin > 3
  if strcmp(prop,'Smallest')
    [sD, sinx] = sort(D,'ascend');
  elseif strcmp(prop,'Largest')
    [sD, sinx] = sort(D,'descend');
  else
    error(['Unkown property ' prop])
  end
  D = sD(1:val);
  idx = sinx(1:val);
end
